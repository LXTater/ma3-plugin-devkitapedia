...
| Badge | Meaning |
|-------|---------|
| `[Vx.x.x][y] VERIFIED` | Tested on hardware, log output confirmed. Build on this. x.x = MA3 Big Version (2.4.2) [y] = verified on what type? [HW(any MA3 hardware with built-in OS (Console, command wing XT, etc.);onPC (with no MA hardware on network.), onPC.VK/CW/DMX/Other(viz-key, command wing, DMX-Key, Other = onPC with PU, node, etc in the network) |
